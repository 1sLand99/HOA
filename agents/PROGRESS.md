...
| HAP 原生 .so 加载 | ✅ | DT_NEEDED 链：libace_napi.z.so stub + NDK libc++_shared + bionic libc |
| 原生 .so C++ 路径解析 | ✅ | `arkui_napi` 仓库 3 处改动：绕过 IsExistedPath、回退 key 匹配、完整路径拼接 |
| NAPI 模块导出 | ✅ | `libentry.so` add(2,3)=5 验证通过，无 Java 预加载 |
...
### 13. HAP 原生 .so 支持 — Phase 1 最小方案（2026-05-29）
...
- 完善 Ability 生命周期回调
- 更多测试 HAP 样本验证
- **Native .so Phase 2**：完整 musl bridge（`agents/native-so-compat.md`），应对依赖其他 OHOS 系统库或 musl 专有符号的复杂 .so
