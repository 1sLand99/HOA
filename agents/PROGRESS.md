...
**里程碑**: Phase 2 原生 .so 支持（2026-05-29），hilog 日志系统迁移（2026-05-30），HDC 调试/部署支持（2026-06-01），拓扑依赖加载（2026-06-02），Pure-ABC 模块集成（2026-06-03），ohbili 白屏定位并解决（2026-06-03），LinyBrowser + ViewPU 修复 + Window API stub（2026-06-08），stateMgmt 构建可靠化（2026-06-08），settings C→Pure-ABC 迁移（2026-06-08），ohtotptoken HAP 全功能适配（2026-06-13）。

HDC daemon 支持 DevEco Studio 直接识别为 OpenHarmony 设备，并通过完整部署流程（shell/file send/bm install）安装和启动 HAP。FilePicker Java 类集成修复完成，`@kit.CoreFileKit` 文件选择器全链路可用。

从 `hoa-6.1` 切换到 `weekly`，基于 ArkUI-X 7.0-Beta1 / OpenHarmony-7.0-Beta1。13 个 fork 仓库通过 harmony-on-android org 管理，`hoa.xml` manifest override。

7 个已测试 HAP 中 6 个正常渲染、1 个部分渲染。ohtotptoken 为首个涉及数据库加密 / HDS titleBar 菜单 / BottomTabBarStyle 的 SDK 13 HAP，攻克了 4 个新的兼容性维度。
...
| Pure-ABC 模块集成 | ✅ | 无 .so 纯 ABC stub 模式，`@ohos.fileio` 全链路验证通过，`export default { ... }` 扁平化导出 |
| `@ohos.settings` Pure-ABC | ✅ | 纯 JS 实现，getValueSync/setValueSync 内存存储 + display/date/domainName 常量，已纳入 GN `ohos_abc("settings")` 构建 |
| 点号分隔模块名 NAPI 解析 | ✅ | `@ohos.file.fs` / `@ohos.file.statvfs` 等点号分隔模块名文件系统路径正确构造 |
| LinyBrowser HAP 渲染 | ✅ | V2 状态管理 + AppStorageV2 时序正常，完整浏览器界面渲染 |
| stateMgmt 构建可靠化 | ✅ | npm install 纳入 GN 管理，输出大小验证，不再出现 3 字节空文件 |
| Window API stub | ✅ | setWindowRectAutoSave / setImmersiveModeEnabledState / setWindowDecorVisible，C++ stub + JS 代理 |
| ohtotptoken HAP | ✅ | TOTP 令牌管理器全功能运行，标题栏菜单按钮（搜索/添加）可点击，搜索栏可见，底部 Tab 正常 |
| RDB 加密剥离 | ✅ | `#if defined(ANDROID_PLATFORM)` 强制 `isEncrypt=false, encryptAlgo=PLAIN_TEXT` |
| KV Store 加密剥离 | ✅ | `#if defined(CROSSPLATFORM)` 强制 `encrypt=false, securityLevel=S1` |
| checkAccessToken 适配 | ✅ | tokenId=0 允许 + OHOS→Android 权限映射 + verifyAccessToken 注册 |
| HDS titleBar 兼容 | ✅ | Proxy + prototype shim → `Navigation.title()` + `Navigation.menus()`，菜单按钮直接渲染 |
| BottomTabBarStyle | ✅ | `globalThis` 暴露 + `@ohos.arkui.component` 桥接 NAPI 模块 |
| stateMgmt 构建简化 | ✅ | 回退到上游 build.py（自带 npm install 自动逻辑），删除多余 GN action |
...
### 23. Pure-ABC 模块集成 — `@ohos.fileio`（2026-06-03）

部分 OHOS 模块在 ArkUI-X/Android 上没有对应的 native 实现（`.so`），但 HAP 会 import 它们。如果模块解析失败，整个 HAP 无法启动。这类模块通过纯 ABC stub（无对应 .so）提供兼容导出。
...
| 模式 | .so | .abc | 注册方式 | 适用场景 |
|------|-----|------|---------|---------|
| HMS stub | 有 | 嵌入 .so | `napi_module_with_js` + `nm_get_abc_code` | 需要 C++ 逻辑或多模块共享 ABC |
| Pure-ABC | 无 | 独立文件 | `systemres/abc/` 自动发现 | 纯 JS 兼容层，无需 native 代码 |
| Native module | 有 | 无 | NAPI `NM_modName` 结构体 | 有真实 native 实现 |

**关键规则 — 扁平化 default export**：运行时通过 `GetExportObjectFromBuffer("default")` 获取模块导出，HAP 中 `import fileio from '@ohos.fileio'` 是 default import。所有函数和类必须以 `export default { func1, func2, Class1, ... }` 形式导出，不能用 `export function` / `export class`（named export）。

**实施步骤**：
1. 创建 JS stub：`plugins/file/fileio/fileio.js`（346 行），使用扁平化 `export default { ... }`
2. 添加 BUILD.gn 目标：`ohos_abc("fileio")` → 编译为 `fileio.abc`
3. `build.sh` 编译 + `sync_arkui_x.sh` 同步到 `app/assets/sys/systemres/abc/`
4. 验证：`adb logcat | grep -E "fileio|RegisterByBuffer"` 确认加载链

**`FAIL: {}` 诊断经验**：`JSON.stringify(Error)` 返回 `{}` 因为 Error 属性不可枚举。实际错误是 `TypeError: fileio.statSync is not a function`，根因是 default export 为 undefined。

详细文档见 `agents/pure-abc-module-integration.md`。

**仓库改动**：

| 仓库 | 改动 | 说明 |
|------|------|------|
| `plugins` | `plugins/file/fileio/fileio.js`（新增） | Pure-ABC stub，346 行 |
| `ace_engine` | `BUILD.gn` +5 行 | `ohos_abc("fileio")` 构建目标 |
| `napi` | `native_module_manager.cpp` +8/-2 | 点号分隔模块名路径修复（Build.gn 与同步） |

### 24. 点号分隔模块名 NAPI 解析修复（2026-06-03）

`@ohos.file.fs` 和 `@ohos.file.statvfs` 等模块名含点号，`native_module_manager.cpp` 的 `FindNativeModuleByDisk` 在构造文件系统路径时未将点号转换为下划线（目录名 `file_fs`，但模块名 `file.fs`），导致 ABC 文件查找失败。

**修复**：引入 `strPathName` 变量用于文件系统路径（点转下划线），保留 `strCutName` 用于缓存 key 匹配（保持点号），两者分别服务于磁盘查找和模块名索引。

改动：`native_module_manager.cpp` 1 个文件，+8/-2 行。

### 25. ohbili HAP 白屏排查（2026-06-03）

`com.wathinst.ohbili`（哔哩哔哩第三方客户端，SDK 9 / ark9.0.0.0）启动后白屏。排查过程：

1. **怀疑 `bundleInfo_ is nullptr`** — 日志显示 `module list size: 0` 和 `bundleInfo_ is nullptr`。经分析，这是 `ScheduleLaunchApplication` 中基于 APK assets 的模块加载返回空列表的**正常行为**（HOA 使用动态 HAP 安装，不走 assets 路径）。`HandleDispatchOnCreate` 有独立的 fallback 路径通过 `GetSplicingModuleName` 构造正确路径读取 `filesDir/hap/` 下的模块。

2. **真因是测试参数错误** — 之前 `adb shell am start` 使用了 `--es bundleName`（代码期望 `--es targetBundle`），且未 `am force-stop` 导致 `onNewIntent` 被调用（DevTestActivity 不处理），`autoLaunch` 参数未生效。

3. **正确启动后 HAP 正常渲染**：Splash 页加载成功（`LoadPage Success`），ArkUI 首帧刷新（`flush first frame messages`），RSSurface 创建并 attach（`AddSurface: surface_count_ = 1`），窗口获焦。已知的权限/accessToken 错误（`checkAccessToken failed`、`requestPermissionsFromUserError`）不阻塞渲染。

**HAP 特征**：`compileMode: "esmodule"`（原 HAP 拼写少 'e'，不影响解析）、`srcEntrance`（SDK 5.0 格式）、targetAPIVersion 9、有 native 依赖但媒体播放功能未验证（ijkplayer .so 需 musl bridge 支持）。

---

### 26. LinyBrowser 白屏修复 — ViewPU + Window API stub（2026-06-08）

LinyBrowser（`meow.liny.browser.cat.uwu`）启动白屏，涉及两个独立问题：

**问题 A — ViewPU/ViewBuildNodeBase 未定义**：`gn clean` 后首次构建，`stateMgmt.js` 只输出 3 字节（应为 ~952KB），编译出的 `stateMgmt.abc` 缺失核心类，导致**所有 HAP** 白屏。根因是 `build.py` 的 npm install 隐藏在脚本内部，GN 不知晓 node_modules 状态。

**解决方案 A**：参照 `ark_breakpoint` 模块，将 stateMgmt 的 npm install 提升为独立 GN action（`state_mgmt_npm_install` + stamp file），`build.py` 移除内部 npm install 逻辑，增加输出文件大小验证（<100KB 则失败）。

**问题 B — Window API 缺失**：`onLoadContent` 调用 `windowStage.setWindowRectAutoSave` 报 `TypeError: undefined is not callable`，阻断后续 `AppStorageV2` 值创建。根因是 `window_stage.js` 是 `WindowStage` 类的 JS 代理，只显式代理了 12 个原生方法，未代理的方法在 JS 端不可见。

**解决方案 B**：在 `js_window.cpp/h` 和 `js_window_stage.cpp/h` 添加 no-op C++ stub，在 `window_stage.js` 代理类中添加 `setWindowRectAutoSave` 代理方法。Window 对象不走 JS 代理，C++ stub 直接可用。

**仓库改动**：

| 仓库 | 改动 | 说明 |
|------|------|------|
| `ace_engine` | `BUILD.gn` + `npm_install_guard.py` + `build.py` | stateMgmt 构建可靠化 |
| `app_framework` | `js_window.cpp/h`、`js_window_stage.cpp/h`、`window_stage.js`、`BUILD.gn` | Window API stub + JS 代理 |

**验证**：LinyBrowser 完整浏览器界面渲染（地址栏 `MeowAddressInput`、底部工具栏），`loadContent` 回调在 `page_router_manager` 创建页面前触发，`AppStorageV2` 值正确创建。

---

### 27. settings 从 C stub 迁移到 Pure-ABC（2026-06-08）

原 `settings_stub.c`（210 行 C）通过 CMake + NDK 编译为 `libsettings_napi.so`，链接 `libarkui_android.so` 解析 NAPI 符号。迁移到 pure-ABC 模式后：

- **源码**：`jsSettings.js`（60 行 JS），`export default { getValueSync, setValueSync, display, date, domainName, wireless, usb }`
- **构建**：GN `ohos_abc("settings")` 目标 + sync 脚本 fallback（GN 对新目标的发现机制存在已知局限，暂时通过 es2abc 手动编译覆盖）
- **加载**：运行时 `dlopen` 失败 → 回退到 `systemres/abc/settings.abc` → `RegisterByBuffer` → `GetExportObjectFromBuffer("default")`
- **删除**：`app/src/main/cpp/settings_stub.c`、`plugins/settings/` 目录已清理

**对比**：

| 方面 | 旧方案 (C stub) | 新方案 (Pure-ABC) |
|------|----------------|-------------------|
| 代码量 | 210 行 C | 60 行 JS |
| 编译 | NDK + 链接 libarkui_android.so | es2abc JS→ABC |
| 产物 | libsettings_napi.so (~9KB) | settings.abc (~1.5KB) |
| 依赖 | CMake 独立编译 | sync 脚本 fallback（GN 就绪后可移除） |

**验证**：hap-example SettingsTest 全部 PASS（getValueSync、setValueSync、overwrite、namespace constants）。

**仓库改动**：

| 仓库 | 改动 | 说明 |
|------|------|------|
| `ace_engine` | `engine/jsSettings.js`（新增）+ `BUILD.gn`（+9 行） | Pure-ABC stub + GN 目标 |
| `HOA` | `sync_arkui_x.sh`（+8 行）、`settings_stub.c`（删除） | fallback 编译 + 清理旧文件 |
| `plugins` | 删除 `settings/` 目录 | 已迁移到 ace_engine |

**已知局限**：~~GN 对新 `ohos_abc` 目标存在发现机制问题（`fileio` 同样未构建，`withenv` 因初始仓库就存在而正常）。根因待查，sync 脚本 fallback 覆盖。~~ **✅ 已修复 (2026-06-10)**：`ohos_abc()` 目标必须被 deps 引用才会被 GN 构建。将 `:settings`、`:fileio`、`:environment` 添加到 `declarative_js_engine`/`declarative_js_engine_ng` 的 deps 后，GN 正常输出三个 .abc 文件。sync 脚本 fallback 已移除。
...
### 28. ohtotptoken HAP 适配 — 数据库加密 / 权限 / HDS 标题栏 / BottomTabBarStyle（2026-06-13）

`smartcityshenzhen.yylx.totptoken`（TOTP 令牌管理器，SDK 13 / API 12）在 HOA 上全功能运行。涉及 4 个维度的问题修复，HAP 本身未做任何修改。

**A — 数据库加密剥离（HUKS 不可用）**：HAP 使用 `encrypt: true, securityLevel: S3/S4` 创建 RDB 和 KV 数据库，底层需 HUKS 硬件加密（Android 上不存在），返回 `DBStatus::INVALID_PASSWD_OR_CORRUPTED_DB`。

| 仓库 | 文件 | 改动 | 分支 |
|------|------|------|------|
| `distributeddatamgr/relational_store` | `napi_rdb_js_utils.cpp` | +18 行：`#if defined(ANDROID_PLATFORM)` → 强制不加密 | `hoa` |
| `distributeddatamgr/kv_store` | `js_util.cpp` | +8 行：`#if defined(CROSSPLATFORM)` → 强制不加密 | `hoa` |

RDB 的 `IsEncrypt()` 检查顺序为 `encryptAlgo != PLAIN_TEXT` → `isEncrypt_` → `!encryptKey_.empty()`，必须全部清空。两仓库已 fork 到 `harmony-on-android` org，manifest `hoa.xml` 已添加 override（共 13 个 fork 仓库）。

**B — checkAccessToken 12100001**：`bundleManager.getBundleInfoForSelf()` 在 Android 上返回 `tokenId=0`。

- `js_ability_access_ctrl.cpp`：`IsTokenIDValid()` 在 `ANDROID_PLATFORM` 上始终返回 true
- `ability_access_ctrl_impl.cpp`：新增权限映射 `DISTRIBUTED_DATASYNC→INTERNET`、`STORE_PERSISTENT_DATA→INTERNET`；普通权限（INTERNET）绕过 JNI + runtime 流程直接授予

**C — HDS 标题栏兼容（最复杂的 JS 层修复）**：OHOS `HdsNavigation().titleBar({ content: { title, menu } })` 在 ArkUI-X Navigation 上不存在。ArkTS 编译器将链式方法编译为模块导出的静态属性访问。

在 `hds_component_mock.js` 中，逐层解决：
1. **Proxy + prototype shim**：`new Proxy(Navigation, {...})` 拦截缺失的 `titleBar` 方法 → 返回自实现的 `_hoaTitleBar`
2. **标题文本**：`_hdsUnwrapTitleBar` 从 HDS 格式 `content.title.mainTitle` 提取，调用已存在的 `Navigation.title()`
3. **菜单按钮**：`Navigation.menus(config.menuItems)` — HDS `content.menu.value` 解包后直接传入 ArkUI 已有 API，渲染为标题栏右上角图标按钮（🔍搜索、➕添加）
4. **死代码清理**：移除 `globalThis.__hoaAppCtx` 事件触发代码（从未被设置，搜索栏显示是 HAP 自身初始状态）

**D — BottomTabBarStyle "undefined is not callable"**：`import { BottomTabBarStyle } from '@kit.ArkUI'` 编译为 `LDEXTERNALMODULEVAR @ohos.arkui.component`，ArkUI-X 无此模块。

- `jsEnumStyle.js`：`globalThis.BottomTabBarStyle` 暴露 ES class
- 新建 `plugins/arkui/component/` 插件（ABC-only NAPI 模块）
- `HoaApplication.kt`：`System.loadLibrary("arkui_component")`

**随附清理**：
- `ace_engine`：回退 `stateMgmt/build.py` 到上游版本，删除 `npm_install_guard.py` — 上游 build.py 已自带 node_modules 缺失时的自动 npm install，外部 GN action 多余且增加合并冲突风险

**验证结果**（uiautomator dump）：标题 "OTP令牌" 显示正常，搜索按钮（`NavigationMenu__MenuItem1__` "搜索密钥"）、添加按钮（`MenuItem2__` "添加密钥"）可点击，搜索栏 EditText 可见，底部 Tab "令牌"/"我的" 正常。
