...
ArkUI-X 源码 (repo 管理, 100+ 仓库)
  + HOA fork 覆盖 (6 个仓库, hoa-weekly 分支)
    → ./build.sh → 编译产物
...
## Step 1: 下载 ArkUI-X 源码

HOA 的 ArkUI-X manifest 仓库位于 `harmony-on-android` 组织，`hoa-weekly.xml` 自动拉取 ArkUI-X 基线并覆盖 6 个定制仓库。
...
# 下载预编译工具链（Clang, GN, Ninja 等）
bash build/prebuilts_download.sh --skip-ssl
```

> `hoa-weekly.xml` 通过 `<include name="default.xml"/>` 拉取 ArkUI-X 全部仓库，再用 `<project>` override 将 6 个定制仓库指向 `harmony-on-android` 组织下的 fork（`hoa-weekly` 分支）。非定制仓库保持跟踪 ArkUI-X 上游。
...
全部修改在 `hoa-weekly` 分支上（基于 `weekly_20260518`）。详细说明见 `docs/ARKUI-X_PATCHES.md`。
...
HOA/
├── manifests/
│   └── hoa-weekly.xml              # HOA manifest（已推送到 harmony-on-android/manifest）
├── scripts/
...
确认网络可访问 `gitcode.com`（manifest URL: `https://gitcode.com/harmony-on-android/manifest`）。如果在受限环境中，可能需要配置代理：
...
- `hoa-weekly.xml` — `include default.xml` + override 6 个定制仓库指向 `harmony-on-android/*` 的 `hoa-weekly` 分支
- 非定制仓库（arkui-x 和 openharmony）保持跟踪上游 `weekly` 基线

更新 manifest 时，在本地 manifest 仓库编辑 `hoa-weekly.xml` 并推送即可。
